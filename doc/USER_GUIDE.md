...
* **Only provider overloads are retried.** Rate limits (HTTP `429` / `RESOURCE_EXHAUSTED`), authentication failures and prompt-too-long errors are not retried — a quota refusal needs operator attention, and the other two are handled elsewhere (history compaction, error reporting). Transient network failures (read timeouts, connection resets) already have their own single retry inside the agent loop.
